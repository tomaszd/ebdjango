...
def get_details_from_single_card(soup, full_url):
    try:
        ...
        # PRICE
    try:
        price = soup.findAll("dt", text="Price Trend")[0].next_sibling.text.split()[0] + " EURO"
    except:
        price = "dziwaczna"
...
def get_singles_details_MCM(cardname=None):
    ...
    rows_with_products = [row for row in soup.findAll("div", {"class": "row no-gutters"}) if row.has_attr("id")]
...
def get_price_trend(lowest_price_single):
    ...
    try:
        ...
        soup = BeautifulSoup(data_for_single.text, features="lxml")
        price_trend = soup.findAll("dt", text="Price Trend")[0].next_sibling.text.split()[0] + " EURO"
    except:
